...
class Harness:
    def __init__(self, deployment: ConfigPaths, settings: AppSettings, analyzer: Path) -> None:
        self.console = ScriptedConsole(["1", "y", "", "", ""])
        ...
        self.workflow = CollectWorkflow(
            ...
            self.console,
            ConfirmPrompt(self.console),
            AnalyzeWorkflow(
                InteractivePackageSelector(self.packages, ChoicePrompt(self.console)),
                AnalysisOptionsPrompt(self.console),
                RedisScopeRunner(self.analyses, analysis_settings, settings.storage),
                self.console,
            ),
        )
